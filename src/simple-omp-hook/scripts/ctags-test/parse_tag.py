...
Function = namedtuple('Function', 'file name signature call')
...
for f in files:
    ...
    for tag in tags:
        ...
        if (func_name.startswith('GOMP_') or func_name.startswith('GOACC_')) and func_name not in [x.name for x in all_tags] and len(fields) > 4:
            ...
            signature = fields[4].replace('signature:', '').replace('\n', '')
            ...
            all_tags.append(Function(f, func_name, signature, args_str))

for tag in all_tags:
    print(tag)

print('Number of functions discovered: ' + str(len(all_tags)))
